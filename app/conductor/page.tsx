    ...
    <div className="flex flex-col h-screen overflow-y-auto no-scrollbar pb-[120px] bg-surface-black px-4 pt-8 [&>*]:shrink-0">
      ...
      <div className="mb-6 flex justify-between items-start">
        <div>
          ...
          <h2 className="text-3xl font-extrabold text-white">Route 500D</h2>
        </div>
        <div className="bg-surface-dark border border-brand-dark px-3 py-1.5 rounded-xl shadow-md text-right">
          <span className="text-[10px] text-gray-400 font-bold uppercase block">Device ID</span>
          <span className="text-white font-mono text-xs">ETM-BLR-042</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-surface-dark p-4 rounded-2xl border border-surface-dark flex flex-col justify-between">
          <p className="text-[10px] text-gray-400 font-bold uppercase">Shift Revenue</p>
          <p className="text-3xl font-black text-white mt-2">₹{totalRevenue}</p>
          <span className="text-[10px] text-emerald-400 font-semibold mt-1">Live Synced</span>
        </div>
        <div className="bg-surface-dark p-4 rounded-2xl border border-surface-dark flex flex-col justify-between">
          <p className="text-[10px] text-gray-400 font-bold uppercase">Change Dispensed</p>
      ...
      <div className="flex bg-surface-dark p-1 rounded-xl mb-6 border border-brand-dark">
        <button 
          ...
          className={`flex-1 py-3 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${activeTab === "validate" ? "bg-brand-dark text-white shadow-md" : "text-gray-400"}`}
        >
          <Scan size={16} /> Validate Passes
        </button>
        <button 
          onClick={() => setActiveTab("cash-sync")}
          className={`flex-1 py-3 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${activeTab === "cash-sync" ? "bg-indigo-900/50 text-indigo-300 shadow-md" : "text-gray-400"}`}
        >
      ...
      <AnimatePresence mode="wait">
        {activeTab === "validate" && (
          ...
          >
            ...
            <div className="bg-surface-dark border border-brand-base p-6 rounded-3xl shadow-lg mb-6 relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-brand-base/10 to-surface-black opacity-40"></div>
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-bold text-white mb-1">Optical & NFC Pass Validator</h3>
                <p className="text-xs text-gray-400 mb-6">Point ETM scanner at commuter's dynamic TOTP QR code.</p>
              ...
              <div className={`p-5 rounded-2xl border shadow-xl ${verificationResult.status === "valid" ? "bg-emerald-950/30 border-emerald-500" : "bg-red-950/30 border-red-500"}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">{verificationResult.commuterName} • {verificationResult.route}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === "cash-sync" && (
          ...
          >
            ...
            <div className="bg-gradient-to-br from-indigo-950/40 via-surface-dark to-surface-black border border-indigo-500/50 p-6 rounded-3xl shadow-lg mb-6">
              <div className="flex items-center gap-3 mb-6">
                <div className="bg-indigo-900/50 p-2 rounded-xl text-indigo-400 border border-indigo-500/30">
                  <SmartphoneNfc size={24} />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white">Cash-to-Wallet Sync</h3>
                  <p className="text-xs text-gray-400">Digitize commuter change instantly.</p>
                </div>
              </div>

              <div className="flex flex-col gap-4 mb-6">
                <div>
                  <label className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block mb-1">Ticket Fare</label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-surface-black text-white rounded-xl py-3 pl-8 pr-4 outline-none border border-surface-dark focus:border-indigo-500 text-lg font-black"
                    />
                  </div>
                </div>
                
                <div>
                  <label className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block mb-1">Cash Note Received</label>
                  <div className="grid grid-cols-4 gap-2 mb-2">
                    {[50, 100, 200, 500].map(amt => (
                      <button 
                        key={amt}
                        onClick={() => setCashReceived(amt)}
                        className={`py-2 rounded-lg text-xs font-bold border transition-colors ${cashReceived === amt ? "bg-indigo-600 border-indigo-500 text-white" : "bg-surface-black border-surface-dark text-gray-400"}`}
                      >
                        ₹{amt}
                      </button>
                    ))}
              ...
              <div className="bg-black/50 p-4 rounded-2xl border border-indigo-900/50 flex justify-between items-center mb-6">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-bold text-white mt-1">User Wallet</p>
                </div>
              </div>
